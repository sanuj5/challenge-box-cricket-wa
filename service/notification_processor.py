...
class NotificationProcessor(BaseProcessor):
    ...
    def send_scheduled_notifications(self):
        ...
        if not bookings or len(bookings) == 0:
            ...
            final_message = "    --------------------------------------------------------------    ".join(
                [
                    f"_*BOOKING {ind + 1}:*_ +{booking.mobile}, {self.db_service.get_user_details(booking.mobile) or ""} --> {',   '.join([self.slots.get(slot).get("title") for slot in sorted(booking.slots)])}"
